...
void loop() {
  ...
  //get humidity data from DHT11
  if (dht::readRawData(dhtPIN, &rawtemperature, &rawhumidity, false) == 0)
  {
    Serial.print("(raw) T: ");
  ...
  char mqttjson[jsonMsg(rawhumidity).length()+1];
  jsonMsg(rawhumidity).toCharArray(mqttjson, jsonMsg(rawhumidity).length()+1);
...
//    char* payload = new char[payloadLen];
    ...
    //MOTOR - OPEN door & blind
    else if (strstr(topic, "/cmd/opendoor") != NULL){
      digitalWrite(motorPIN, HIGH);
      delay(2000);
      digitalWrite(motorPIN, LOW);
    }
      
    //MOTOR - CLOSE door & blind
    else if (strstr(topic, "/cmd/closedoor") != NULL){
      digitalWrite(motorPIN, HIGH);
      delay(2000);
      digitalWrite(motorPIN, LOW);
    }
    
    //WHITE LED - room lights ON
    else if (strstr(topic, "/cmd/lightson" != NULL){
      digitalWrite(lightingPIN, HIGH);
    }
    
     //WHITE LED - room lights OFF
    else if (strstr(topic, "/cmd/lightsoff" != NULL){
      digitalWrite(lightingPIN, LOW);
    }
    
    //RED LED - rooom lcd projector ON
    else if (strstr(topic, "/cmd/lcdon" != NULL){
      digitalWrite(projectorPIN, HIGH);
    }
      //RED LED - rooom lcd projector OFF
    else if (strstr(topic, "/cmd/lcdoff" != NULL){
      digitalWrite(projectorPIN, LOW);
    }
    
    //ORANGE LED - HVAC system: turn ON heating mode 
    else if (strstr(topic, "/cmd/heatingon" != NULL){
      digitalWrite(heatingPIN, HIGH);
    }
    
    //ORANGE LED - HVAC system: turn OFF heating mode 
    else if (strstr(topic, "/cmd/heatingoff" != NULL){
      digitalWrite(heatingPIN, LOW);
    }
    
     //ORANGE LED - HVAC system: ADJUST heating level 
    else if (strstr(topic, "/cmd/heatingadj" != NULL){
    }
    
    //GREEN LED - HVAC system: turn ON cooling mode 
    else if (strstr(topic, "/cmd/coolingon" != NULL){
      digitalWrite(coolingPIN, HIGH);
    }
    
    //GREEN LED - HVAC system: turn OFF cooling mode 
    else if (strstr(topic, "/cmd/coolingoff" != NULL){
      digitalWrite(coolingPIN, LOW);
    }
    
    //GREEN LED - HVAC system: ADJUST cooling level 
    else if (strstr(topic, "/cmd/coolingadj" != NULL){
    }
...
String jsonMsg(int16_t hum) {
  
  //get temp value
  //dtostrf(getTemp(),1,2, tempbuff);
  
  //get temp value
  //dtostrf(getHumidity(),1,2, humbuff);
